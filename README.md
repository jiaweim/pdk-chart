...
#### Shape Lookup Fallback Chain

1. Explicit per-series shape (`getSeriesShape`).
2. If null and `autoPopulateSeriesShape` is on → pull next shape from `DrawingSupplier`.
3. Otherwise → `defaultShape`.

> [!TIP]
>
> To configure the `defaultShape` property, you must disable `autoPopulateSeriesShape` first, which defaults to `true`.
