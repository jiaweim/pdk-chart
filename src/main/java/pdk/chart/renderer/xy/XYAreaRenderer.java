 ...
 * <p>
 * XYAreaRenderer：功能全面的 Area Renderer。能够画线、点、面积，支持渐变、FillPaint，
 * 并通过累计整条曲线形成一个完整的填充区域，适合需要丰富绘制效果或大数据集的场景。
 * XYAreaRenderer2：精简的纯 Area Renderer。每个数据点独立构造并填充一个局部多边形，
 * 不支持线和点等附加功能，实现简单，但功能较少，更适合作为基础实现或进行二次开发。
 */
