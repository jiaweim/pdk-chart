...
public class PeriodAxisDemo3 extends ApplicationFrame {
    ...
    private static Chart createChart(IntervalXYDataset dataset) {
        BarChart chart = new BarChart(dataset, "Day", AxisType.PERIOD,
                "Temperature", "Maximum Temperature");
        chart.setDomainCrosshairVisible(true);
        chart.setRangeCrosshairVisible(true);

        PeriodAxis xAxis = (PeriodAxis) chart.getDomainAxis();
        xAxis.setAutoRangeTimePeriodClass(Day.class);
        ...
        info[1] = new PeriodAxisLabelInfo(Day.class, new SimpleDateFormat("E"),
                new RectangleInsets(2.0, 2.0, 2.0, 2.0),
        ...
        xAxis.setLabelInfo(info);

        return chart;
    }

    private static IntervalXYDataset<String> createDataset() {
        TimeSeries<String> s1 = new TimeSeries<>("Temperature");
